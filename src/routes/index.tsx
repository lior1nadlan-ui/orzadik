...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    ...
    links: [
      ...
      // The video itself is withheld until idle (see HomePage) and never counts
      // toward LCP; the poster is what a phone actually paints first.
      {
        rel: "preload",
        as: "image",
...
/**
 * The hero video: a fast-cycling montage of the live catalog, generated by
 * scripts/campaign-video.html for the Meta traffic campaign and re-cut here
 * for the site (1080x990, no baked-in brand card — see the long comment at
 * the <video> element for why the two differ). Drops into the same full-bleed
 * box the photo crossfade used, no layout change needed. No audio track
 * exists in the source file, so there is nothing to mute.
 */
...
function HomePage() {
  ...
  // paint; with <source> children present at first render, autoPlay would
  // override preload="metadata" and immediately stream the ~3MB file,
  // competing with hero-poster.webp on mobile. So the sources are withheld
  // from the initial render (poster only) and attached after the browser goes
  // idle — requestIdleCallback, with a ~1200ms setTimeout fallback.
  //
  // Reduced-motion users never get sources attached and never download the
  // video at all — they keep the poster as a still image, a complete hero on
  // its own.
  const [heroSourcesReady, setHeroSourcesReady] = useState(false);
  // WCAG 2.2.2 (Pause, Stop, Hide): the video plays on its own, indefinitely,
  // and nothing on the page could stop it. prefers-reduced-motion covers the
  ...
  // below is that stop. It starts unpaused, and the pause survives for the
  // life of the page — deliberately not persisted, since it is a reading aid
  // for this visit rather than a setting.
  ...
  // Once the <source> children are in the DOM, load() re-selects the resource
  // so the just-attached sources are picked up; autoPlay + muted then starts
  // the loop on its own.
  useEffect(() => {
    if (heroSourcesReady) {
  ...
  // The pause control acts on the element directly rather than on autoPlay,
  // so pausing mid-loop freezes on the current frame instead of jumping back
  // to the poster.
  useEffect(() => {
    if (!heroSourcesReady) return;
    if (heroPaused) heroVideoRef.current?.pause();
    else heroVideoRef.current?.play()?.catch(() => {});
  }, [heroSourcesReady, heroPaused]);
  ...
  return (
    <>
      {/* 1. Hero — full-bleed video.
          The frame is the full viewport and the type sits ON the footage
          instead of in a card floating over it — the owner's standing note
          against a boxed, cropped hero (see the scrim comment below for the
          "תמחק את הקוביה הלבנה" history). */}
      <section className="relative">
        {/* The campaign video montage, looping. Replaces the 2026-08-16 photo
            crossfade (which replaced a bespoke 16:9 HyperFrames loop before it —
            those files are gone from public/media/ now that this one lives
            there instead). Decorative, exactly as both predecessors were: the
            footage carries no information the type below does not state in
            words, so it is hidden from the accessibility tree rather than
            announced as video content. This is what the accessibility statement
            (§4.5) declares — keep them in sync.

            The source is close to square (1080x990 — see below for why it
            isn't the campaign's native 4:5), so it drops into the same
            full-bleed box the photo crossfade used: object-cover crops the
            sides on a narrow phone and crops top/bottom on a wide desktop,
            with no reason to bias off center since the mosaic has no single
            focal point.

            WHY THIS ISN'T THE CAMPAIGN'S OWN FILE, RE-USED. The Meta creative
            (scripts/campaign-video.html's normal output) bakes in a permanent
            gold-bordered "אור זרוע לצדיק" card, dead-centre, for the ad —
            exactly right when the video IS the message on a feed, exactly
            wrong stacked under this page's OWN headline two lines below,
            which already does that job with real contrast guarantees. Cutting
            the card out live (network access to Supabase Storage is not
            available in every environment this repo is built in, so
            regenerating the montage without it isn't always an option) means
            working from the rendered master: the card's fixed band
            (rows 1042-1657 of the original 2160x2700 export, detected by
            scanning for its gold border across sampled frames) is cropped out
            and the remaining top/bottom halves are stacked, which is why the
            delivered file is shorter/less-portrait than the ad's own 4:5. */}
        ...
        >
          <video
            ...
            playsInline
            preload="metadata"
            className="absolute inset-0 h-full w-full object-cover bg-cream"
          >
            {/* Sources are attached only after the browser is idle (see the
                effect above) so they never compete with the LCP poster on first
                paint, and are skipped entirely for reduced-motion users. WebM
                (VP9) first; browsers that can't play it fall back to the MP4. */}
            {heroSourcesReady && (
              <>
        ...
        {/* Scrim.
            ...
            were doing was hazing the middle of the frame. The 0.84 band stays
            exactly where it is regardless of what plays behind it — that value
            is not tuned to any one asset, it is the arithmetic floor that holds
            against a pure-white frame, which is worth restating now that the
            footage itself is a mosaic of product photos on white backgrounds:
            this is the one hero fill so far where that worst case is not
            hypothetical. Lightening it to fit today's asset is the trade this
            file has already been burned by once. */}
        <div
          aria-hidden="true"
        ...
        <div className="absolute inset-x-0 bottom-0 flex flex-col items-center px-5 pb-10 text-center sm:px-6 md:pb-14">
          ...
          {/* Rendered only once the video is actually running: before sources
              attach there is no motion to stop, and for a reduced-motion
              visitor heroSourcesReady never becomes true, so no control appears
              for a hero that is a still poster. 44px hit area, matching the
              header's controls. */}
          {heroSourcesReady ? (
            <div className="mt-6 flex items-center justify-center">
              <button
                ...
                aria-label={heroPaused ? "הפעל את הסרטון" : "עצור את הסרטון"}
