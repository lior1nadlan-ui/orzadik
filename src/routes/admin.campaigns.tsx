...
function AdminCampaigns() {
  ...
  const resetComposer = () => {
    setEditingId(null); setSubject(""); setIntro(""); setPicked([]); setProductQuery("");
  };
  ...
  const openEdit = async (id: string) => {
    try {
      ...
      setPicked((c.content?.products ?? []).map((p: any) => ({
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      save({ data: { id: editingId ?? undefined, subject, intro, product_ids: picked.map((p) => p.id) } }),
    onSuccess: () => {
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => tickMutation.mutate()} disabled={tickMutation.isPending} className="gap-2">
            <Send className="h-4 w-4" /> {tickMutation.isPending ? "שולח..." : "שלח קבוצה עכשיו"}
          </Button>
          <Button onClick={openNew} className="gap-2"><Plus className="h-4 w-4" /> קמפיין חדש</Button>
        </div>
      </div>
      ...
      <div className={`rounded-lg border bg-card overflow-x-auto transition-opacity ${isFetching ? "opacity-60" : ""}`}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {campaigns.length === 0 && (
              <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">אין קמפיינים עדיין.</td></tr>
            )}
            {campaigns.map((c: any) => (
              <tr key={c.id} className="border-t">
                <td className="p-3 max-w-xs"><div className="line-clamp-2">{c.subject}</div></td>
                <td className="p-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs ${STATUS_CLS[c.status] ?? ""}`}>
      ...
      {/* Composer */}
      <Dialog open={composerOpen} onOpenChange={(v) => { setComposerOpen(v); if (!v) resetComposer(); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editingId ? "עריכת קמפיין" : "קמפיין חדש"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
              <Label htmlFor="c-subject">נושא *</Label>
              <Input id="c-subject" maxLength={150} value={subject} onChange={(e) => setSubject(e.target.value)} />
            ...
            <div>
              ...
              <Textarea id="c-intro" rows={5} maxLength={2000} value={intro} onChange={(e) => setIntro(e.target.value)} />
            </div>
            <div>
              <Label htmlFor="c-products">מוצרים ({picked.length}/{MAX_PRODUCTS})</Label>
              {picked.length > 0 && (
                <div className="flex flex-wrap gap-2 my-2">
                  {picked.map((p) => (
                    <span key={p.id} className="inline-flex items-center gap-1 rounded-full border px-2 py-1 text-xs">
                      {p.name.slice(0, 30)}
                      <button type="button" onClick={() => togglePick(p)} aria-label={`הסר ${p.name}`}>
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                  ))}
                ...
                <div className="mt-2 max-h-48 overflow-y-auto rounded-md border divide-y">
                  {productResults.map((p: any) => (
                    ...
                    >
                      ...
                      {picked.some((x) => x.id === p.id) && <span className="text-accent text-xs">נבחר ✓</span>}
                    </button>
                  ))}
                </div>
              )}
            ...
            <>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex flex-wrap gap-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs ${STATUS_CLS[detail.status] ?? ""}`}>
                ...
                <div className="rounded-md border overflow-hidden">
                  ...
                  {previewHtml ? (
                    ...
                    <div className="p-8 text-center text-muted-foreground">טוען תצוגה מקדימה...</div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  ...
                  {detail.status === "sending" && (
                    <Button variant="outline" className="gap-2" disabled={tickMutation.isPending} onClick={() => tickMutation.mutate()}>
