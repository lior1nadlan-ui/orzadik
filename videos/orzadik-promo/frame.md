...
description: >
  ...
  as bloom / panel / tile underprint, Assistant display + Assistant sans + JetBrains Mono data,
...
typography:
  ...
  # — display ramp (Assistant 400, tight, negative tracking) —
  ledger-title:{ fontFamily: "Assistant", cqw: 1.55, weight: 400, lineHeight: 1.15 }
  strand-title:{ fontFamily: "Assistant", cqw: 1.7, weight: 400, lineHeight: 1.1 }
  headline-sm:{ fontFamily: "Assistant", cqw: 2.9, weight: 400, lineHeight: 1.0 }
  headline:{ fontFamily: "Assistant", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.005em" }
  date-rail:{ fontFamily: "Assistant", cqw: 5.0, weight: 400, lineHeight: 0.96, tracking: "-0.005em" }
  display-it:{ fontFamily: "Assistant", cqw: 7.0, weight: 400, italic: true, lineHeight: 1.04, tracking: "-0.005em" }
  numeral-md:{ fontFamily: "Assistant", cqw: 7.5, weight: 400, lineHeight: 0.92, tracking: "-0.01em" }
  display:{ fontFamily: "Assistant", cqw: 14.6, weight: 400, lineHeight: 0.86, tracking: "-0.018em" }
  numeral-jumbo:{ fontFamily: "Assistant", cqw: 28.0, weight: 400, lineHeight: 0.84, tracking: "-0.04em" }
...
- **Fonts** — already set to **Assistant** (display) / **Cormorant Garamond** (body); ignore any preset font name lingering in prose.
...
The voice is three faces in rigid roles: **Assistant** (weight 400, tight line-height,
negative tracking) carries every display, numeral, and quote from 40px to 720px+; **Assistant** carries
...
- **Assistant 400** display (tight, negative-tracked); **Assistant** body + micro-labels; **JetBrains Mono** data.
...
- **Squint** — one Assistant moment dominates at 3–6× its neighbor; the sun bloom centers the eye.
...
Two ramps. The **reading/data ramp** (Assistant body 0.85cqw ink, micro-labels in px, JetBrains Mono
data) carries copy + chrome; the **display ramp** (Assistant `ledger-title` 1.55cqw →
...
- **Assistant is weight 400 only** (its contrast is the weight signal), tight line-height (0.84–1.06), negative tracking; **micro-labels uppercase Assistant 600, ≥0.16em**; **mono for all numerals/dates**. No bold serif, no second text color, no mono in body/display.
...
headline-sm. **Focal** a single huge Assistant `numeral-jumbo` (≈28cqw) dominating, with a
...
**Focal** a 2-line Assistant **italic** `display-it` quote in ink, centered, under an
...
yellow-panel, micro-label, headline. **Focal** an Assistant `headline` in ink sitting **on
...
- Set every line in **ink**; use **Assistant 400** (tight, negative-tracked) for display, **Assistant** body, **JetBrains Mono** for all numerals/dates.
- Make every separator a **1px ink hairline** (soft variant for dense rows); flood a **yellow panel** for poster moments (ink on top).
- Keep micro-labels **uppercase Assistant 600, 0.16–0.32em**; pin the **pagenum** bottom-right.
...
- No second text color; no bold Assistant; no inverted ink grounds.
...
- **Type** — Assistant 400 tight negative-tracked, fit-to-measure; micro-labels uppercase 0.16em+; mono numerals; ≥1.4cqw floor.
...
- **Assistant + Assistant + JetBrains Mono via Google Fonts.** CJK: Smiley Sans (display) / Noto Serif SC (body) / Noto Sans SC (labels); italic serif and mono tabular figures have no exact Hanzi equal — keep ledger dates Latin.
